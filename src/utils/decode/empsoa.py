    ...
    def beam_search(self, src_seq, max_dec_step):
        ...
        def beam_decode_step(
            ...
            def predict_word(
                ...
                gate = nn.Sigmoid()(self.model.fusion(torch.cat([dec_output, user_state.unsqueeze(1).expand_as(dec_output), agent_state.unsqueeze(1).expand_as(dec_output)], dim=-1)))
                dec_output = dec_output + gate * user_state.unsqueeze(1).expand_as(dec_output) + (1-gate) * agent_state.unsqueeze(1).expand_as(dec_output)
        ...
        with torch.no_grad():
            ...
            conv_len = [len(item) for item in src_seq["input_txt"]]
            mask_src = enc_batch.data.eq(config.PAD_idx).unsqueeze(1)
            user_seq_mask = (src_seq["mask_input"].data.eq(config.USR_idx).float().unsqueeze(1)) * (1-mask_src.float())
            agent_seq_mask = (src_seq["mask_input"].data.eq(config.SYS_idx).float().unsqueeze(1)) * (1-mask_src.float())
            ...
            utt_cls_index = src_seq["x_cls_index"]
            
            # -----------------Commonsense relations----------------
            if config.csk_feature:
                cog_cls = []
                for r in self.model.rels[:-1]:
                    cog_cls.append(self.model.cog_encoder(src_seq[r]))
                emo_cls = self.model.emo_encoder(src_seq["x_react"])
            # -------------------------------------------------------

            self_other_graph = self.model.construct_graph(src_emb, cog_cls, emo_cls, utt_cls_index, conv_len)
            x, user_state, _ = self.model.self_other_interaction_1(self_other_graph, src_seq["user_mask"], conv_len, 'user')
            x, user_state, _ = self.model.self_other_interaction_1(x, src_seq["user_mask"], conv_len, 'user')

            x, agent_state, _ = self.model.self_other_interaction_1(self_other_graph, src_seq["agent_mask"], conv_len, 'agent')
            x, agent_state, _ = self.model.self_other_interaction_1(x, src_seq["agent_mask"], conv_len, 'agent')

            user_ctx = self.model.user_fusion(torch.cat([src_enc, user_state.unsqueeze(1).expand_as(src_enc)], dim=-1))
            agent_ctx = self.model.agent_fusion(torch.cat([src_enc, agent_state.unsqueeze(1).expand_as(src_enc)], dim=-1))

            user_ref_ctx = self.model.ctx2user_cross_attention(src_enc, user_ctx, user_ctx, user_seq_mask)
            agent_ref_ctx = self.model.ctx2agent_cross_attention(src_enc, agent_ctx, agent_ctx, agent_seq_mask)

            gate = nn.Sigmoid()(self.model.reg_fusion(torch.cat([user_ref_ctx, agent_ref_ctx], dim=-1)))
            cog_ref_ctx = gate * user_ref_ctx + (1-gate) * agent_ref_ctx
